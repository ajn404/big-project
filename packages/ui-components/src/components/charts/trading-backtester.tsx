...
import { createChart, IChartApi, ISeriesApi, LineStyle, ColorType, CandlestickData, Time, CandlestickSeries } from 'lightweight-charts';
...
// 定义 TradingBacktesterComponent 的 props 接口
interface TradingBacktesterComponentProps {
  symbol?: 'SPY' | 'QQQ' | 'VTI';
...
interface TradingBacktesterProps {
  ...
  colors?: TradingBacktesterComponentProps['colors'];
}
...
export const TradingBacktesterComponent: React.FC<TradingBacktesterComponentProps> = ({
    ...
    try {
      ...
      if (chartRef.current && candlestickSeriesRef.current) {
        ...
        if (candlestickSeriesRef.current) {
          console.log('Series object:', candlestickSeriesRef.current);
          console.log('Has setMarkers:', 'setMarkers' in candlestickSeriesRef.current);
          console.log('Type of setMarkers:', typeof (candlestickSeriesRef.current as any).setMarkers);

          if (typeof (candlestickSeriesRef.current as any).setMarkers === 'function') {
            (candlestickSeriesRef.current as any).setMarkers(markers);
          } else {
            console.error('setMarkers is not a function on the series object');
          }
        }
      }
  ...
  }, [backgroundColor, textColor, upColor, downColor, borderUpColor, borderDownColor]);

  // Load initial data
  useEffect(() => {
    runBacktest();
  }, [currentSymbol, currentStrategy]);
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <select
            ...
            value={currentSymbol}
            onChange={(e) => setCurrentSymbol(e.target.value as any)}
          >
...
// 包装组件 - 提供明确的 props 类型
function TradingBacktester({ symbol = 'SPY', strategy = 'grid', children, colors }: TradingBacktesterProps) {
  return <TradingBacktesterComponent symbol={symbol} strategy={strategy} colors={colors} />;
}

// 自动注册组件
const RegisteredTradingBacktester = createAutoRegisterComponent({
